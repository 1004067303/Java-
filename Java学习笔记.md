...
自JDK5.0开始提供了代表线程池的接口：ExecutorService
...
基本的通信架构有两种：C-S架构（Client-Server），B-S架构（Browser-Server）
...
32位，八位成一个十进制用点隔开 形成IP地址（点分十进制表示法），如

11000000 10101000 00000001 01000010 转换为IP地址为： 192.168.1.66

因为32位，不够全球使用，所以引入了IPV6

### IPV6

128位，号称可以为地球每一粒沙子编号，分为八段表示，每段每四位编码成一个十六进制位标识，数之间用冒号（：）分开

### ![image-20241120105031059](D:\JAVA\JavaDemo\笔记图片\image-20241120105031059.png) IP域名

一般我们访问网站都是通过域名进行访问的，如（http://www.baidu.com) 这就是域名

对于域名，我们计算机内部自带有dns服务器（域名解析器），主要作用就是将域名解析成为ip地址，便于我们访问，如果我们输入的域名在我们的电脑没有解析出来，那么就会转发到运营商服务器上进行解析，然后再返回到本机，最后解析完成。
...
192.168.开头的就是常见的局域网地址，范围为：192.168.0.0--192.168.255.255，专门为组织机构内部使用
...
static InetAddress getByName（String host）				根据ip地址或者域名，返回一个InetAddress对象

String getHostName（）								     获取该ip地址对象对应的主机名

String getHostAddress（）								 获取该ip地址对象的ip地址信息

boolean isReachable（int timeout）						在指定毫秒内，判断主机与该ip对应的主机是否能连通
...
我们自己开发的程序一般选择使用注册端口，并且一个设备中不能出现两个程序的端口号一样，否则出错

## 通信协议

网络上通信的设备，事先规定的连接规则，以及传输数据的规则被称为网络通信协议

开放式网络互联标准：OSI网络参考模型，这是全球网络互联标准

TCP/IP网络模型：事实上的国际标准，大家都用的这个

| OSI网络参考模型 | TCP/IP网络模型  | 各层对应              | 面向操作                                                     |
| --------------- | --------------- | --------------------- | ------------------------------------------------------------ |
| 应用层          |                 |                       |                                                              |
| 表示层          |                 |                       |                                                              |
| 会话层          | 应用层          | HTTP、FTP、SMTP...... | 应用程序需要关注的：浏览器，邮箱，程序员一般在这一层进行开发 |
...
发送方不管对方是否在线，数据在中间丢失也不管，如果接收方收到数据也不返回确认，故是不可靠的（半双工）

但是通信效率高：一般用于语音通话，视频直播等
...
void receive（DatagramPacket p）			使用数据包接受数据
...
TCP主要有三个步骤实现可靠传输：三次握手建立连接，传输数据进行确认，四次挥手断开连接（全双工）

通信效率相对不高：一般使用于网页、文件下载、支付等场景
...
可以用来对方法进行测试，是第三方公司开源出来的（很多开发工具都集成了Junit框架，如 IDEA）
...
1、将Junit框架的jar包导入到项目中（注意：IDEA集成了Junit框架，不需要我们自己手工导入）
